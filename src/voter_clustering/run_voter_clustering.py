...
def run_clustering_and_save(
    ...
    assignments_out_csv.parent.mkdir(parents=True, exist_ok=True)
    ...
    if mode == "A":
        ...
        for dc in dao_vals:
            ...
            if len(sub) < mode_a_min_sub_rows():
    ...
    kmeans_metrics_path = assignments_out_csv.parent / "kmeans_sweep_metrics.csv"
    pd.DataFrame(metrics_rows).to_csv(kmeans_metrics_path, index=False)
    log(f"Saved k-means sweep metrics: {kmeans_metrics_path}")
