...
from load_data import fit_numeric_preprocessor, load_dataset, normalise_columns, select_numeric_columns, split_by_voter
...
def main() -> None:
    ...
    raw_df = load_dataset(Path(args.dataset_csv))
    train_raw, valid_raw = split_by_voter(raw_df, train_frac=0.8, seed=args.seed)
    preprocessor = fit_numeric_preprocessor(train_raw)
    train_df = normalise_columns(train_raw, preprocessor=preprocessor)
    valid_df = normalise_columns(valid_raw, preprocessor=preprocessor)
    ...
    for epoch in range(args.epochs):
        ...
        for batch in tqdm(train_loader, desc=f"train {epoch+1}/{args.epochs}"):
            ...
            if not torch.isfinite(loss):
                raise RuntimeError("Non-finite loss detected (NaN/Inf). Check numeric preprocessing and input data.")
            optimizer.zero_grad()
            loss.backward()
