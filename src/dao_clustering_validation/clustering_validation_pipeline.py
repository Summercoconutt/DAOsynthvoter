...
# Fallback if INPUT_PATH missing (whole_pipeline layout: repo_root/data/processed/...)
_FALLBACK_REL = Path(__file__).resolve().parents[2] / "data" / "processed" / "dao_feature_table.csv"
FALLBACK_CSV = _FALLBACK_REL if _FALLBACK_REL.exists() else (
    Path(__file__).resolve().parent.parent / "DAO_Clustering" / "scripts" / "data" / "processed" / "dao_feature_table.csv"
)

OUTPUT_DIR = Path(__file__).resolve().parents[2] / "outputs" / "clustering_validation"
# Step 7-style bundle (heatmap, distributions, assignments, PCA): mirrors dissertation demands
CLUSTER_RESULTS_DIR = Path(__file__).resolve().parents[2] / "outputs" / "cluster_results"
...
def apply_runtime_path_overrides() -> None:
    """Allow whole_pipeline / CI to inject paths via environment (pathlib-friendly)."""
    global INPUT_PATH, OUTPUT_DIR, CLUSTER_RESULTS_DIR, FALLBACK_CSV
    if os.environ.get("PIPELINE_DAO_FEATURES_INPUT", "").strip():
        INPUT_PATH = Path(os.environ["PIPELINE_DAO_FEATURES_INPUT"])
    if os.environ.get("PIPELINE_DAO_FEATURES_FALLBACK_CSV", "").strip():
        FALLBACK_CSV = Path(os.environ["PIPELINE_DAO_FEATURES_FALLBACK_CSV"])
    if os.environ.get("PIPELINE_CLUSTERING_VALIDATION_OUT", "").strip():
        OUTPUT_DIR = Path(os.environ["PIPELINE_CLUSTERING_VALIDATION_OUT"])
    if os.environ.get("PIPELINE_CLUSTER_RESULTS_DIR", "").strip():
        CLUSTER_RESULTS_DIR = Path(os.environ["PIPELINE_CLUSTER_RESULTS_DIR"])
